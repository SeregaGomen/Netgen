...
void STLView::calcRadius()
{
    ...
    for (int i = 2; i <= static_cast<STLGeometry*>(geometry)->GetNP(); i++)
        for (auto j = 0; j < 3; j++)
        {
            minX[j] = static_cast<STLGeometry*>(geometry)->GetPoint(i)(j) < minX[j] ? static_cast<STLGeometry*>(geometry)->GetPoint(i)(j) : minX[j];
            maxX[j] = static_cast<STLGeometry*>(geometry)->GetPoint(i)(j) > maxX[j] ? static_cast<STLGeometry*>(geometry)->GetPoint(i)(j) : maxX[j];
        }
    for (auto i = 0; i < 3; i++)
